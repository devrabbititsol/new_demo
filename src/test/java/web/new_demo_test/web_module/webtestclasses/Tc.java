...
import web.new_demo_test.web_module.webpageclasses.Technology_screen;
...
public class Tc extends BaseClass {
	...
	public void Technology_screenTest(int i) throws Exception {

	 try{
		Constants.TOTAL_TC = Constants.TOTAL_TC + 1;
		Constants.IS_TESTCASE = true; Constants.iS_WEB = true;
		int datasetScreencount = 3;
		Technology_screen objTechnology_screen = PageFactory.initElements(driver, Technology_screen.class);
		testLogHeader(test, "Verify Technology_screen page");
		String text1 = objTechnology_screen.clkaSecurityAssessment_402005();
		if(text1.equalsIgnoreCase(configFileObj.getProperty("SecurityAssessment"+ i + datasetScreencount))){
			printSuccessLogAndReport(test, logger,  "Clicked on : " + configFileObj.getProperty("SecurityAssessment"+ i + datasetScreencount));
			printSuccessLogAndReport(test, logger,  "Validated Link Text : " + configFileObj.getProperty("SecurityAssessment"+ i + datasetScreencount));
		} else {
			printFailureLogAndReport(test, logger,  "Link Text is not displayed  : " + configFileObj.getProperty("SecurityAssessment"+ i + datasetScreencount));
		}
	...
	public void screensTest() throws Exception {
		for(int datasets = 1; datasets <= Technology_screen.datasetsLength; datasets++) {
			isElementDispalyed = true;			
			setUP();
			if(isElementDispalyed) { Home_screenTest(datasets);}
			if(isElementDispalyed) { Services_screenTest(datasets);}
			if(isElementDispalyed) { Technology_screenTest(datasets);}
			tearDown();
		}	}
